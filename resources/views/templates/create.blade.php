...
<div class="max-w-3xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 mt-2">Create Message Template</h1>
    </div>
        ...
        <form action="{{ route('templates.store') }}" method="POST">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="title" id="title" required value="{{ old('title') }}" placeholder="e.g. Course Registration Open" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
                ...
                <div>
                    ...
                    <select name="template_category_id" id="template_category_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        ...
                        @foreach($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('template_category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <textarea name="body" id="body" rows="8" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm font-mono" placeholder="Dear [Student Name], your course registration for [Programme] is now open.">{{ old('body') }}</textarea>
                </div>
                ...
                <div class="flex justify-end gap-3 border-t border-gray-100 pt-5">
                    ...
                    <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-md hover:bg-indigo-500">Save Template</button>
                </div>
            </div>
        </form>
    </div>
